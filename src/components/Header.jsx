...
function Header() {
  ...
  return (
    <header className=' w-full h-[70px] z-10 bg-white dark:bg-slate-900 shadow flex items-center justify-between px-5 md:px-[12%] '>
      {/* logo */}
      <img src={theme === 'dark' ? logodark : Logow} alt="" className='cursor-pointer w-[9rem]' />

      {/* Menu Responsive */}
      <div onClick={() => setMenu(!menu)}
        className='flex md:hidden bg-[#f5c32c] z-10 text-slate-800 text-xl shadow dark:bg-slate-800 rounded mx-4 items-center justify-center cursor-pointer w-[40px] h-[40px] dark:text-[#f5c32c]'>
        {!menu ? <AiOutlineMenu /> : <AiOutlineClose />}
      </div>

      {/* navbar */}
      <div className={`${menu ? 'flex md:flex ' : ' hidden md:flex'} absolute z-10 md:static  top-[71px] dark:top-[71.3px] md:top-0  right-0 md:right-0 flex flex-col md:flex-row shadow md:shadow-none   md:items-center bg-white md:bg-none  dark:bg-slate-900 w-[80%] md:w-auto  h-[100vh] md:h-auto `}>
        ...
        {/* DarkMode */}
        <div className="bg-[#f5c32c] text-slate-800 z-10 shadow dark:bg-slate-800 rounded mx-4 flex items-center justify-center cursor-pointer w-[40px] h-[40px] dark:text-[#f5c32c] md:mt-0 mt-3"
